...
@Repository
interface CandleFifteenRepository : CrudRepository<CandleFifteenImpl?, SymbolDateId?> {
    @Query("select c from CandleFifteenImpl c where c.symbol = ?1 and c.time > ?2 and c.time < ?3")
