...
@Repository
interface CandleHourlyRepository : CrudRepository<CandleHourlyImpl?, SymbolDateId?> {
    @Query("select c from CandleHourlyImpl c where c.symbol = ?1 and c.time > ?2 and c.time < ?3")
