...
class MinswapV2ClassifierIT: Base() {
    ...
    fun computeSwaps_SingleTransaction_SingleSwap_1() {
        ...
        val subscription = blockFlux.subscribe { block: Block ->
            ...
            val qualifiedTxMap = swapProcessor.qualifyTransactions(block.header.headerBody.slot, block.transactionBodies, block.transactionWitness)
        ...
        val orderedComputedSwaps = allSwapDTOS.sortedBy { it.slot }.sortedBy { it.txHash }.sortedBy { it.amount1 }.sortedBy { it.amount2 }
        val orderedKnownSwaps = knownSwapDTOS.sortedBy { it.slot }.sortedBy { it.txHash }.sortedBy { it.amount1 }.sortedBy { it.amount2 }
    ...
    fun computeSwaps_SingleTransaction_QtySwaps() {
        ...
        val knownSwapDTOS = listOf(
            SwapDTO(swapTxHash, 132216341, DexEnum.MINSWAPV2.code, "lovelace", "64f7b108bd43f4bde344b82587655eeb821256c0c8e79ad48db15d1844454449", BigDecimal.valueOf(2061343905), BigDecimal.valueOf(9956660000), DexOperationEnum.BUY.code),
            SwapDTO(swapTxHash, 132216341, DexEnum.MINSWAPV2.code, "lovelace", "64f7b108bd43f4bde344b82587655eeb821256c0c8e79ad48db15d1844454449", BigDecimal.valueOf(9862068966), BigDecimal.valueOf(48129095766), DexOperationEnum.SELL.code),
        )
    ...
    fun computeSwaps_QtyTransaction_QtySwaps_2() {
        val knownSwapDTOS = listOf(
            SwapDTO("504122667db93a394e78abfabb1f2d40e1205e1a7c14cbdb1b67a63b5e6e57bb", 132291896, DexEnum.MINSWAPV2.code, "lovelace", "279c909f348e533da5808898f87f9a14bb2c3dfbbacccd631d927a3f534e454b", BigDecimal.valueOf(50641844 ), BigDecimal.valueOf(23218 ), DexOperationEnum.BUY.code),
            SwapDTO("6b403ef7286497c85edf7ebe55b14877bedac0efbbbda7a5dafbbf077501e606", 132291807, DexEnum.MINSWAPV2.code, "lovelace", "279c909f348e533da5808898f87f9a14bb2c3dfbbacccd631d927a3f534e454b", BigDecimal.valueOf(3056441804 ), BigDecimal.valueOf(1400000  ), DexOperationEnum.BUY.code),
            SwapDTO("b73ce1ee2242a09c9b5540f56a9a8e3295f6f95feefb85dbdf7b718890ccbd04", 132291574, DexEnum.MINSWAPV2.code, "lovelace", "279c909f348e533da5808898f87f9a14bb2c3dfbbacccd631d927a3f534e454b", BigDecimal.valueOf(419999450  ), BigDecimal.valueOf(188404 ), DexOperationEnum.SELL.code),
            SwapDTO("12c374cf7cb4fac3eba2cfe95579634cf5a18b8d12e503baee9b1123a8b6e899", 132291432, DexEnum.MINSWAPV2.code, "lovelace", "279c909f348e533da5808898f87f9a14bb2c3dfbbacccd631d927a3f534e454b", BigDecimal.valueOf(419999450 ), BigDecimal.valueOf(188450 ), DexOperationEnum.SELL.code),
            )
        ...
        val subscription = blockFlux.subscribe { block: Block ->
            ...
            println("All swaps: $allSwapDTOS")
        }
        val untilSlot = knownSwapDTOS.maxOf { it.slot }
        var runningSwapsCount = 0
        runBlocking {
            while(true) {
                if (allSwapDTOS.isNotEmpty() && allSwapDTOS.size > runningSwapsCount) {
                    println("Running # swaps: ${allSwapDTOS.size}, Up to slot: ${allSwapDTOS.last().slot}, syncing until: $untilSlot")
                    runningSwapsCount = allSwapDTOS.size
                }

                if (allSwapDTOS.isNotEmpty() && allSwapDTOS.last().slot > untilSlot) {
                    break
                }
                delay(100)
            }
        ...
        // Sort them exactly the same way since multiple swaps per tx, and multiple tx per block
        val orderedComputedSwaps = allSwapDTOS
    ...
    fun computeSwaps_SingleTransaction_Type4_Deposit() {
        ...
        val orderedKnownSwaps = MinswapV2ClassifierTest.filterSwapsByType(knownSwapDTOS, allQualifiedTxMap, listOf(0, 4))
            .sortedBy { it.txHash }.sortedBy { it.amount1 }.sortedBy { it.amount2 }
        ...
        orderedKnownSwaps.zip(orderedComputedSwaps).forEach {
            ...
            assertTrue { it.first.asset2Unit == it.second.asset2Unit }
            assertTrue { it.first.amount1 == it.second.amount1 }
            /* Discussions with Minswap confirmed these calculations are correct however the values taken from their
               history page are slightly off and they may have roundings errors, here we only check if the values are close */
            assertTrue { it.first.amount2 - it.second.amount2 < BigDecimal.valueOf(10) }
            assertTrue { it.first.operation == it.second.operation }
        }
    }
