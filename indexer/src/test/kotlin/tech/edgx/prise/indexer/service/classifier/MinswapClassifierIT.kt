...
class MinswapClassifierIT: Base() {
    ...
    fun computeSwaps_SingleTransaction_1() {
        ...
        */
        val knownSwapDTOS = listOf(
            SwapDTO("79a56a719258ca13715bb82c24c869590c37859bba8329667a52f414950c13d1", 112541775, DexEnum.MINSWAP.code, "lovelace", "8cfd6893f5f6c1cc954cec1a0a1460841b74da6e7803820dde62bb78524a56", BigDecimal.valueOf(465245143), BigDecimal.valueOf(11341315232), DexOperationEnum.BUY.code),
        )
        ...
        blockSync.startSync(startPoint,
            object : BlockChainDataListener {
                override fun onBlock(era: Era, block: Block, transactions: MutableList<Transaction>) {
                    ...
                    val v1Swaps = allSwapDTOS.filter { it.dex==DexEnum.MINSWAP.code }
                    assertTrue(v1Swaps.isNotEmpty())
                    assertEquals(knownSwapDTOS.size, v1Swaps.size)
                    knownSwapDTOS.zip(v1Swaps).forEach {
    ...
    fun computeSwaps_SingleTransaction_2() {
        ...
        */
        val knownSwapDTOS = listOf(
            SwapDTO("b3e436dbe8af67247b1def8412fe172a905d98c106c08eb2ffe3c2fa91180c9d", 112553479, DexEnum.MINSWAP.code, "lovelace", "279c909f348e533da5808898f87f9a14bb2c3dfbbacccd631d927a3f534e454b", BigDecimal.valueOf(1789513117), BigDecimal.valueOf(701549), DexOperationEnum.SELL.code),
        )
        ...
        blockSync.startSync(startPoint,
            object : BlockChainDataListener {
                override fun onBlock(era: Era, block: Block, transactions: MutableList<Transaction>) {
                    ...
                    val v1Swaps = allSwapDTOS
                        .filter { it.dex==DexEnum.MINSWAP.code }
                        .filter { swap -> knownSwapDTOS.map { it.txHash}.contains(swap.txHash) }
                    assertTrue(v1Swaps.isNotEmpty())
                    assertEquals(knownSwapDTOS.size, v1Swaps.size)
                    knownSwapDTOS.zip(v1Swaps).forEach {
                        println("Comparing: ${it.first} to ${it.second}")
    ...
    fun computeSwaps_SingleTransaction_4() {
        ...
        val subscription = blockFlux.subscribe { block: Block ->
            ...
            assertEquals(knownSwapDTOS.size, allSwapDTOS.size)
            knownSwapDTOS.zip(allSwapDTOS).forEach {
                println("Comparing: ${it.first} to ${it.second}")
                assertTrue { it.first.txHash == it.second.txHash }
                assertTrue { it.first.slot == it.second.slot }
                assertTrue { it.first.dex == it.second.dex }
    ...
    fun computeSwaps_SingleTransaction_5() {
        ...
        val knownSwapDTOS = listOf(
            SwapDTO(txHash, 112501470, DexEnum.MINSWAP.code, "lovelace", "1d7f33bd23d85e1a25d87d86fac4f199c3197a2f7afeb662a0f34e1e776f726c646d6f62696c65746f6b656e", BigDecimal.valueOf(270000000), BigDecimal.valueOf(1045332878), DexOperationEnum.SELL.code),
        )
