...
import tech.edgx.prise.indexer.domain.DailyCandles
...
class DailyCandleRepository(dataSource: DataSource) {
    private val log = LoggerFactory.getLogger(javaClass::class.java)
    private val database = Database.connect(dataSource)
    var batch_size = 500

    val Database.candles get() = this.sequenceOf(DailyCandles)
        ...
        candles.chunked(batch_size).forEach { cc ->
            cc.forEach { c ->
                database.useTransaction {
                    database.bulkInsertOrUpdate(DailyCandles) {
                        item {
                            set(it.symbol, c.symbol)
                            set(it.time, c.time)
                            set(it.open, c.open)
            ...
            log.debug ("Batch persisted daily candles" )
        }
    }
    ...
    fun persist(candles: List<CandleDTO>) {
        ...
        log.debug("Persisted daiily candles")
    }

    fun getCandlesAtTime(fromTime: Long): List<CandleDTO> {
        val latestCandles = database.useConnection { conn ->
            val sql = "SELECT symbol, MAX(time) as time, open, high, low, close, volume FROM candle_daily " + //, '1D' as resolution
    ...
    fun getNCandlesBetweenTimes(from: Long, to: Long): List<LatestCandlesView> {
        return database.useConnection { conn ->
            val sql = "SELECT symbol,time,open,high,low,close,volume, '1D' as resolution " +
                    "FROM candle_daily " +
                    "WHERE time >= ? and time < ?"
            conn.prepareStatement(sql).use { statement ->
    ...
    fun addContinuationCandles(candleDtg: LocalDateTime, candleSymbolsMade: List<String>) {
        ...
        database.useConnection { conn ->
            ...
            val sql = String.format(
                "insert into candle_daily (time, symbol, open, high, low, close, volume) " +
